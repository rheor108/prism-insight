...
2026-09-10: research의 웹 검색은 Codex 내부 도구로 실행하며, 최종 JSON의
`calls`는 빈 배열로 제한합니다. 일반 MCP 호출용 지시와 검색용 지시를 분리했습니다.
내부 웹 도구를 외부 요청으로 잘못 반환하면 `[CODEX_WEB_PROTOCOL]`을 남기고
최대 2회 교정합니다. 실제로 실행하지 않은 도구 결과를 만들지 않으며, 기존 단계
시간·반복 제한 안에서만 재시도합니다. 다른 미등록 도구나 주문 도구는 계속 거부합니다.
